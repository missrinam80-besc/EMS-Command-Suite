  ...
  return (
    <main className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-6 py-8 md:px-10 lg:px-12">
      ...
      <section className="rounded-[1.75rem] border border-[var(--color-line-strong)] bg-[radial-gradient(circle_at_top_left,_rgba(82,210,255,0.16),_transparent_34%),linear-gradient(145deg,_var(--color-hero-start),_var(--color-hero-end))] p-8 text-[var(--color-hero-ink)] shadow-[0_30px_80px_rgba(4,28,44,0.24)]">
        ...
        <h1 className="mt-3 text-4xl font-semibold tracking-[-0.04em]">Richtlijnen en procedures</h1>
      </section>

      <section className="rounded-[1.25rem] border border-[var(--color-line)] bg-[var(--color-surface)] p-4">
        <form className="grid gap-3 md:grid-cols-4">
          <input name="q" defaultValue={params.q ?? ""} placeholder="Zoek op titel of samenvatting" className="rounded-xl border border-[var(--color-line)] bg-white px-3 py-2 text-sm md:col-span-2" />
          <select name="category" defaultValue={params.category ?? ""} className="rounded-xl border border-[var(--color-line)] bg-white px-3 py-2 text-sm">
          ...
          {canManage ? (
            <select name="status" defaultValue={params.status ?? "all"} className="rounded-xl border border-[var(--color-line)] bg-white px-3 py-2 text-sm">
          ...
          <button type="submit" className="rounded-full bg-[var(--color-accent)] px-4 py-2 text-sm font-semibold text-[var(--color-ink)] md:col-span-4 md:justify-self-start">Filteren</button>
        </form>
      </section>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <article className="rounded-[1.5rem] border border-[var(--color-line)] bg-[var(--color-surface)] p-6">
            ...
            <form action={createHandbookCategoryAction} className="mt-4 space-y-3">
              <input name="code" placeholder="code" className="w-full rounded-xl border border-[var(--color-line)] bg-white px-3 py-2 text-sm" required />
              <input name="label" placeholder="Naam" className="w-full rounded-xl border border-[var(--color-line)] bg-white px-3 py-2 text-sm" required />
              <textarea name="description" placeholder="Beschrijving" className="h-24 w-full rounded-xl border border-[var(--color-line)] bg-white px-3 py-2 text-sm" />
              <input type="number" name="sortOrder" defaultValue={100} className="w-32 rounded-xl border border-[var(--color-line)] bg-white px-3 py-2 text-sm" />
          ...
          <article className="rounded-[1.5rem] border border-[var(--color-line)] bg-[var(--color-surface)] p-6">
            ...
            <form action={createHandbookArticleAction} className="mt-4 space-y-3">
              <select name="categoryId" className="w-full rounded-xl border border-[var(--color-line)] bg-white px-3 py-2 text-sm" required>
              ...
              <label className="flex items-center gap-2 text-sm text-[var(--color-muted)]"><input type="checkbox" name="isActive" defaultChecked />Actief</label>
              <button type="submit" className="rounded-full bg-[var(--color-accent)] px-4 py-2 text-sm font-semibold text-[var(--color-ink)]">Artikel opslaan</button>
            </form>
          </article>
        </section>
      ) : null}
      ...
      <section className="space-y-4">
        {articles.map((article) => (
          <details key={article.id} className="rounded-[1.25rem] border border-[var(--color-line)] bg-[var(--color-surface)] p-4">
            ...
            <div className="mt-4"><Link href={`/handboek/${article.slug}`} className="rounded-full border border-[var(--color-line)] px-4 py-2 text-sm font-semibold text-[var(--color-ink)]">Open artikel</Link></div>
            {canManage ? (
              <div className="mt-4 grid gap-4 lg:grid-cols-2">
                <form action={updateHandbookArticleAction} className="space-y-2 rounded-xl border border-[var(--color-line)] p-3">
                  <input type="hidden" name="id" value={article.id} />
                  <select name="categoryId" defaultValue={article.categoryId} className="w-full rounded-xl border border-[var(--color-line)] bg-white px-3 py-2 text-sm">{categories.map((category) => <option key={category.id} value={category.id}>{category.label}</option>)}</select>
